...
interface HubListFilterProps {
    ...
    /** 数据为空时的提示内容 */
    noDataHint?: string
    itemExtra?: (key: API.PluginsSearch) => ReactNode
}
/** @name 插件列表筛选条件数据组 */
export const HubListFilter: React.FC<HubListFilterProps> = memo((props) => {
    const {wrapperClassName, loading, groupList, selecteds, onSelect, isHidden, noDataHint, itemExtra} = props
...
export const OnlineOptFooterExtra: React.FC<OnlineOptFooterExtraProps> = memo((props) => {
    ...
    return (
        <div className={styles["online-opt-footer-extra"]}>
            {starLoading ? (
                <LoadingOutlined className={styles["loading"]} />
            ) : (
                <div
                    className={classNames(styles["like-operate"], {
                        [styles["like-operate-active"]]: !!info.is_stars
                    })}
                    onClick={onStar}
                >
                    {info.is_stars ? <SolidThumbupIcon /> : <OutlineThumbupIcon />}
                    <span>{info.starsCountString || ""}</span>
                </div>
            )}
            <div className={styles["divider-style"]}></div>
            {downloadLoading ? (
                ...
                <div className={styles["download-operate"]} onClick={onDownload}>
                    <OutlineClouddownloadIcon />
                    <span>{info.downloadedTotalString || ""}</span>
                </div>
            )}
        </div>
    )
...
export const OwnOptFooterExtra: React.FC<OwnOptFooterExtraProps> = memo((props) => {
    ...
    const onDownload = useMemoizedFn((e) => {
        e.stopPropagation()
        if (!info.uuid) {
            yakitNotify("error", "错误，未获取插件可用信息")
            return
        }
        if (downloadLoading) return
    ...
    return (
        <div className={styles["own-opt-footer-extra"]}>
            <YakitButton
                type='text2'
...
interface RecycleOptFooterExtraProps {
    ...
    execDelInfo?: YakitPluginOnlineDetail[]
    /** 当前正在执行还原的插件UUID队列 */
    execRestoreInfo?: YakitPluginOnlineDetail[]
    onDel: (data: YakitPluginOnlineDetail) => void
    onRestore: (data: YakitPluginOnlineDetail) => void
}
/** @name 回收站单项-删除和还原 */
export const RecycleOptFooterExtra: React.FC<RecycleOptFooterExtraProps> = memo((props) => {
    const {isLogin, info, execDelInfo = [], execRestoreInfo = [], onDel, onRestore} = props
    ...
    const restoreLoading = useMemo(() => {
        if (!execRestoreInfo) return false
        const findIndex = execRestoreInfo.findIndex((ele) => ele.uuid === info.uuid)
        if (findIndex > -1) return true
        return false
    }, [info, execRestoreInfo])
    ...
    return (
        <div className={styles["recycle-opt-footer-extra"]}>
            {delLoading ? (
                <LoadingOutlined className={styles["del-loading"]} />
            ) : (
                <YakitButton type='text2' icon={<OutlineTrashIcon />} onClick={onRemove} />
            )}
            <YakitButton icon={<OutlineDatabasebackupIcon />} onClick={onReduction} loading={restoreLoading}>
                还原
            </YakitButton>
        </div>
    )
